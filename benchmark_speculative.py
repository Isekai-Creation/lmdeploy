...
from lmdeploy import turbomind as _tm_mod
_tm = _tm_mod._tm
...
class BenchmarkRunner:
    ...
    def run_test_scenario(
        ...
        if disable_baseline and not use_speculation:
            ...
            tm_build_id = _tm.build_id() if hasattr(_tm, "build_id") else "unknown"
            results["system"] = {
                "gpu_name": self.gpu_name,
                "gpu_memory_total_gb": self.gpu_memory_total,
                "timestamp": datetime.now().isoformat(),
                "turbomind_build_id": tm_build_id,
            }
        ...
        gen_configs = [
            GenerationConfig(
                ...
                bad_token_ids=bad_token_ids if bad_token_ids is not None else [],
            )
...
def main():
    ...
    build_id_fn = getattr(_tm, "build_id", None)
    build_id = build_id_fn() if callable(build_id_fn) else "unknown"
    ...
    if args.scenario in ["all", "single"]:
        ...
        scenarios.extend(
            [
                ...
                # VLLM-matched scenarios (batch_size=8, context_length=8192, max_new_tokens=24576, session_len=32768)
                {
                    "scenario_name": "Speculative_Batch8_Context8K_1tokens_VLLM",
                    "batch_size": 8,
                    "context_length": 8192,
                ...
                {
                    "scenario_name": "Speculative_Batch8_Context8K_3tokens_VLLM",
                    "batch_size": 8,
                    "context_length": 8192,
                    "max_new_tokens": 24576,
                    "use_speculation": True,
                ...
                {
                    "scenario_name": "Speculative_Batch8_Context8K_4tokens_VLLM",
                    "batch_size": 8,
                    "context_length": 8192,
                    "max_new_tokens": 24576,
                    "use_speculation": True,
                ...
                {
                    "scenario_name": "Speculative_Batch8_Context8K_5tokens_VLLM",
                    "batch_size": 8,
                    "context_length": 8192,
                    "max_new_tokens": 24576,
                    "use_speculation": True,
