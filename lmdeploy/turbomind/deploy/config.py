...
def config_to_dict(config):
    ...
    if not config:
        return dict()
    assert isinstance(
        config, (ModelConfig, AttentionConfig, LoraConfig)
...
class ModelConfig:
    ...
    qk_rope_dim: int = 0
    v_head_dim: int = 0
    # DSA (Dynamic Sparse Attention) for DeepSeek-V32
    index_topk: int = 0
...
class TurbomindModelConfig:
    ...
    def update_from_engine_config(self, config: TurbomindEngineConfig):
        ...
        if hasattr(config, "hf_overrides") and config.hf_overrides:
            ...
            if hf_overrides.get("rope_scaling"):
                ...
                rope_param = self.attention_config.rope_param or RopeParam(
                    type="", base=0, dim=0
                )
                rope_param.type = override_params.get("rope_type", "")
                if (
                    rope_param.type == "yarn"
                    and "original_max_position_embeddings" in override_params
                ):
                    rope_param.factor = (
                        self.attention_config.max_position_embeddings
                        / override_params["original_max_position_embeddings"]
                    )
                    rope_param.max_position_embeddings = override_params[
                        "original_max_position_embeddings"
                    ]
                else:
                    rope_param.factor = override_params.get("factor", 1.0)
                    rope_param.max_position_embeddings = override_params.get(
        ...
        if config.rope_scaling_factor:
            ...
            rope_param.max_position_embeddings = (
                self.attention_config.max_position_embeddings
            )
    ...
    def from_dict(cls, config: dict = {}):
        ...
        return TurbomindModelConfig(
            ...
            attention_config=config_from_dict(
                AttentionConfig, _cfg["attention_config"]
            ),
