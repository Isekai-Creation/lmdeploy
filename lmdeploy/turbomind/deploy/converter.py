...
def get_output_model_registered_name_and_config(
    model_path: str, model_format: str, dtype: str, group_size: int
):
    ...
    """
    print(
        f"DEBUG(get_output_model_registered_name_and_config): initial group_size={group_size}"
    )
    register_name = "tm"
    ...
    print(
        f"DEBUG(get_output_model_registered_name_and_config): model_arch={model_arch}, model_config={model_config}"
    )
    ...
    if dtype == "bfloat16" and not has_bf16:
        logger.warning(
            "data type fallback to float16 since "
    ...
    print(
        f"DEBUG(get_output_model_registered_name_and_config): before _get_and_verify_max_len, model_config={model_config}"
    )
    session_len = _get_and_verify_max_len(model_config, None)
    print(
        f"DEBUG(get_output_model_registered_name_and_config): session_len={session_len}"
    )
    ...
    if model_format in ["awq", "gptq"]:
        ...
        print(
            f"DEBUG(get_output_model_registered_name_and_config): before group_size assignment for awq/gptq, group_size={group_size}"
        )
        group_size = 128 if group_size == 0 else group_size
        print(
            f"DEBUG(get_output_model_registered_name_and_config): after group_size assignment for awq/gptq, group_size={group_size}"
        )
    ...
    if model_arch in ["GptOssForCausalLM", "DeepseekV32ForCausalLM"]:
        weight_type = dtype
    ...
    print(
        f"DEBUG(get_output_model_registered_name_and_config): returning config.model_config.group_size={config.model_config.group_size}, session_len={config.model_config.session_len}"
    )
...
def get_tm_model(
    ...
    input_model = INPUT_MODELS.get(input_model_name)(
        model_path=model_path, tokenizer_path=model_path, input_policy=input_policy
    )
    ...
    output_model = OUTPUT_MODELS.get(output_model_name)(
        input_model=input_model, cfg=tm_cfg, model_cls=Transformer, out_dir=out_dir
    )
