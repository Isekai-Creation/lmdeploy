...
class BaseOutputModel(ABC):
    ...
    def __init__(
        ...
        self.repeat_kv = 0
        if (
            self.attn_tp_size > self.model_config.kv_head_num
    ...
    def export_weight(self, param: torch.Tensor, name: str) -> None:
        ...
        if self.to_file:
            if torch.is_floating_point(param):
                torch_type = _weight_dtype_map(
                    self.model_config.weight_type, torch.float16
                )
            ...
            assert weight_type in ["float16", "bfloat16", "int4", "fp8", "e2m1"]
    ...
    def save_split(
        self, tensor: torch.Tensor, name: str, split_dim=None, split_num=1, copy=False
    ) -> None:
        """Save split.
        ...
        if split_dim is not None:
            ...
            if tensor.shape[split_dim] % split_num != 0:
                raise RuntimeError(
                    f"{name}: shape={list(tensor.shape)}, split_num={split_num}"
                )
    ...
    def export(self) -> None:
        ...
        pbar = tqdm(
            total=num_layer, desc="Convert to turbomind format", leave=self.to_file
        )
        self.export_config()
