...
This copy of `ENGINE.md` lives under `LM/lmdeploy_drift` and should be read together with `LM/lmdeploy_drift/ENGINE_TODOS.md`. All file paths referenced below (e.g. `src/turbomind/...`, `lmdeploy/messages.py`) are relative to this `lmdeploy_drift` workspace. DriftEngine is the non‑speculative TurboMind backend described here; speculative decoding (EAGLE/EAGLE3/SpecPV) remains scoped to `EAGLE_TODOS.md` and `SPECPV_TODO.md`.
